...
/// device already demonstrated for read-only queries. Waits are bounded and pump the run loop,
/// so delegate callbacks land on the same thread and nothing blocks forever.
///
...
public final class TransactionController {
    ...
    public func query(cmd: UInt16, payload: [UInt8] = [], timeout: TimeInterval? = nil) -> QueryOutcome {
        ...
        let sent: RFCOMMTransport.SentFrame
        do {
            sent = try transport.send(cmd: cmd, payload: payload)
        } catch {
        ...
        log(String(format: "TX 0x%04X seq=0x%02X payload=%@", cmd, sent.sequence, Hex.string(payload)))

        let deadline = Date().addingTimeInterval(limit)
        pendingRequest = (cmd: cmd, sequence: sent.sequence)
        ...
        guard let response else {
            log(String(format: "RX 0x%04X seq=0x%02X timeout after %.1fs", cmd, sent.sequence, limit))
            return QueryOutcome(requestCmd: cmd, requestSequence: sent.sequence, responseSequence: nil, status: nil, payload: [], outcome: .timedOut)
        }
        ...
        case .missing:
            return QueryOutcome(requestCmd: cmd, requestSequence: sent.sequence, responseSequence: response.sequence, status: nil, payload: response.payload, outcome: .malformedResponse("响应 payload 为空，无 status 字段"))
        case .rejected(let status):
            return QueryOutcome(requestCmd: cmd, requestSequence: sent.sequence, responseSequence: response.sequence, status: status, payload: response.payload, outcome: .rejected(status: status))
        case .ok(let status):
            return QueryOutcome(requestCmd: cmd, requestSequence: sent.sequence, responseSequence: response.sequence, status: status, payload: response.payload, outcome: .ok)
        }
    }
